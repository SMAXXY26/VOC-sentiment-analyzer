...
function ConfidenceBar({ value }: { value?: number }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <div className="w-14 h-1 rounded-full bg-white/[0.05] overflow-hidden">
    ...
    <tr className="border-b border-white/[0.04]">
      {[220, 80, 60, 60, 70, 40, 50].map((w, i) => (
        <td key={i} className="px-4 py-3">
          <div className="h-3 rounded-full bg-white/[0.04] animate-pulse" style={{ width: w }} />
        </td>
      ))}
    </tr>
  );
  ...
  if (!items.length && !loadingMore) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        ...
        <p className="text-slate-600 text-xs mt-1">Run the pipeline to populate data</p>
      </div>
    );
  }
  ...
  return (
    <div>
              ...
              <>
                <tr
                  ...
                  className={`border-b border-white/[0.03] cursor-pointer transition-all duration-150 ${
                    ...
                    expanded === i
                      ? "bg-white/[0.04] border-l-2 border-l-indigo-500/50"
                      : "hover:bg-white/[0.025] hover:border-l-2 hover:border-l-indigo-500/30"
                  }`}
                >
                  <td className="px-4 py-2.5 max-w-[200px]">
                    <p className="truncate text-slate-200 text-xs">{item.summary}</p>
                  </td>
                  <td className="px-4 py-2.5">
                    <span className="text-xs text-slate-400">{item.category}</span>
                    {item.subcategory && <span className="text-[10px] text-slate-600 block">{item.subcategory}</span>}
                  </td>
                ...
                {expanded === i && (
                  <tr key={`${i}-detail`} className="border-b border-white/[0.04]">
                    <td colSpan={7} className="px-5 py-4 bg-white/[0.02]">
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 animate-fade-in">
                        {item.raw_text && (
                          <div className="lg:col-span-2">
                            <p className="text-[10px] font-mono uppercase tracking-[0.15em] text-slate-600 mb-1.5">Raw Text</p>
                            <p className="text-slate-300 text-xs italic leading-relaxed border-l-2 border-indigo-500/30 pl-3">
                        ...
                        {item.emotions?.length > 0 && (
                          <div>
                            <p className="text-[10px] font-mono uppercase tracking-[0.15em] text-slate-600 mb-1.5">Emotions</p>
                        ...
                        {item.feature_requests?.length > 0 && (
                          <div>
                            <p className="text-[10px] font-mono uppercase tracking-[0.15em] text-slate-600 mb-1.5">Feature Requests</p>
      ...
      {!hasMore && items.length > 0 && (
        <p className="text-center text-[11px] text-slate-700 py-4">All {items.length} items loaded</p>
      )}
    </div>
  );
}
